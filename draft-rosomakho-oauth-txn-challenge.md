...
a deployment MAY use mutual-TLS {{!MTLS=RFC8705}} instead where all participants support it. The mechanism in use
determines how the agent key is identified in a confirmation (`cnf`) claim {{!POP-KEY=RFC7800}}:

DPoP {{DPOP}}:
: The agent key is identified by its JWK Thumbprint {{!JWK-THUMBPRINT=RFC7638}} in the `jkt` confirmation method.
  The agent demonstrates possession by including a DPoP proof in the `DPoP` header field, and the access token has
  `token_type` `DPoP`. This is the mandatory-to-implement mechanism.

mutual-TLS {{MTLS}}:
: The agent key is the key of a client certificate, identified by the certificate SHA-256 thumbprint in the
  `x5t#S256` confirmation method. The agent demonstrates possession over a mutual-TLS connection, and the access
  token has `token_type` `Bearer`.

The protected resource observes the agent key when the agent proves possession on the request that triggers the
challenge, and records its confirmation in the challenge `cnf` claim ({{challenge-claims}}). The agent proves
...
Key-bound profile:
: The challenge contains a `cnf` claim. The authorization server MUST sender-constrain the issued access token to
  the agent key it identifies, and the protected resource MUST verify proof of possession of that key when the
  token is presented. This is the default profile for high-impact or non-idempotent operations and is the profile
...
upgrade one by inserting it. An authorization server MUST honor the profile indicated by the validated challenge.
...
To obtain a key-bound challenge ({{assurance-profiles}}), the agent additionally proves possession of its agent
key on the request, using one of the mechanisms in {{agent-key-pop}}, so that the protected resource can observe
and record the key. An agent that cannot prove possession of a key obtains only a relay-profile challenge.
...
`cnf`:
: Confirmation claim {{POP-KEY}} identifying the agent key the protected resource confirmed on the request that
  triggered the challenge, using the `jkt` member for DPoP or `x5t#S256` for mutual-TLS (see {{agent-key-pop}}).
  Its presence selects the key-bound profile and its absence the relay profile (see {{assurance-profiles}}). When
  present, the authorization server MUST sender-constrain the issued access token to this key, and the protected
  resource MUST verify proof of possession of it when the access token is presented.
...
authorization for a specific operation. In the relay profile, sender-constrained access tokens, for example using
DPoP {{DPOP}} or mutual-TLS {{MTLS}}, MAY be used and are RECOMMENDED for high-impact operations. In the
key-bound profile, the access token MUST be sender-constrained to the agent key identified by the challenge `cnf`
claim: the authorization server sets the access token's `cnf` to that value -- the `jkt` member for DPoP or
`x5t#S256` for mutual-TLS -- as described in {{agent-key-pop}}.
...
In the key-bound profile the access token is sender-constrained to a single agent key, so the agent that
presents it MUST be the agent whose key is bound by the `cnf` claim; an intermediate agent that cannot prove
possession of that key cannot present the token. Delegating the challenged operation to a different agent
therefore requires re-binding the authorization to that agent's key, as described in {{delegation}}.
...
{{MTLS}}. In the key-bound profile, the agent MUST prove possession of the agent key when presenting the access
token: with DPoP, by presenting the token under the `DPoP` authentication scheme together with a DPoP proof; with
mutual-TLS, over a mutual-TLS connection authenticated with the bound certificate.
...
* in the key-bound profile, the agent proves possession of the agent key, and the access token's `cnf` confirmation
  matches that key -- the `jkt` of the DPoP proof key, or the `x5t#S256` of the mutual-TLS client certificate --
  and equals the `cnf` value the protected resource recorded in the challenge (see {{agent-key-pop}});
...
sender-constrained to a single agent key and cannot be relayed to and presented by a different agent. Rather than
weaken the binding, the authorization is re-established at each hop using OAuth 2.0 Token Exchange
{{OAUTH-TOKEN-EXCHANGE}}.

A downstream agent presents the access token it received to the authorization server's token endpoint as the
...
agent's key. Each hop is therefore independently sender-constrained, and the `act` chain records the delegation.

## Propagation Within the Protected Resource Trust Domain {#propagation}

The access token is consumed at the protected resource and is not intended to be forwarded to other components as
a bearer credential. When a protected resource must propagate the authorized-transaction context to downstream
services within its own trust domain as it fans the operation out, it does so after validating the access token,
for example by issuing a transaction token {{?TXN-TOKENS=I-D.ietf-oauth-transaction-tokens}} carrying the same
`txn` value through its trust domain's token service. This keeps the externally presented access token
sender-constrained while propagating context internally.
...
In the key-bound profile, the access token issued for the challenged operation is sender-constrained to the agent
key recorded in the challenge `cnf` claim: the protected resource records the key, the authorization server binds
the access token to it, and the protected resource verifies proof of possession when the token is presented (see
{{agent-key-pop}}). A captured or relayed access token is therefore useless to any party that cannot prove
possession of the agent key. This is stronger than the short-lifetime and single-use mitigations the relay
profile relies on, and protects the leg between the agent and the protected resource — which the agent traverses
through components the protected resource does not control. Because the binding key is named in the
protected-resource-signed challenge, a relaying agent cannot substitute its own key. Protected resources SHOULD
require the key-bound profile for high-impact or non-idempotent operations.
